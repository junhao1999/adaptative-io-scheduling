...
DESCRIPTION
	This file simulates the armed bandit with a single access pattern.
...
labels = getExperimentLabels(args.io_nodes, args.processes, args.test_type, args.strided, args.operation, args.request_size)

# Get the metrics from those labels
observations = getObservations(args.io_nodes, args.processes, args.operation, labels)
...
for action in actions:
	observed_bandwidth[action] = np.mean(observations[action])

	logging.info('TWINS {:4d}(ms) - {}MB/s'.format(action, observed_bandwidth[action]))
...
while True:
	...
	reward = observations[action][random.randint(0, len(observations[action]) - 1)]

	# Remove the observation to avoid repetition
	observations[action].remove(reward)

	total += reward

	log.append([action, reward, total])
	...
	if args.alpha is None:
		...
		Q[action] = Q[action] + (1.0 / float(N[action])) * (reward - Q[action])
	else:
		# If we have an alpha we should use the Nonstationary formula
		Q[action] = Q[action] + float(args.alpha) * (reward - Q[action])
...
summary['experiment']['io_nodes'] = int(args.io_nodes)
summary['experiment']['processes'] = int(args.processes)
summary['experiment']['test_type'] = int(args.test_type)
summary['experiment']['strided'] = int(args.processes)
summary['experiment']['operation'] = args.operation
summary['experiment']['request_size'] = int(args.request_size)
...
for action in actions:
	summary['bandit']['reward']['summary'][action] = observed_bandwidth[action]
